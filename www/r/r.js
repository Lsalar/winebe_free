                     ...
                     add: function(_el = document.getElementsByTagName('body')[0], icono = '<i class="las la-feather"></i>', texto='Aguarde...')
                     {
                        ...
                        var   _aside = $r.create('aside');
                              ...
                              _aside.innerHTML = ' <span class="precarga_contenedor">\
                                                      <span class="precarga_icono">'+icono+'</span>\
                                                      <span class="precarga_texto">'+texto+'</span>\
                                                   </span>';
...
/* _________________________________________________________________________________________________________
   ...
   $r.ajax2 = function(_obd) 
   {
   		...
   		if(_psa == true)
   		{
			...
			if(_obd.secure == true)
				a.setRequestHeader("RdigitalSeg", btoa('RdigitalWinebe'));
   ...
   $r.ajax_file  = function(url, metodo, datos, fileinput)
   {
      var _esp;
      var _ipos; // esto es solo para el uso del chat
      if(typeof datos == "object")
      {
         _esp = datos.get("h");
      }
      else
      {
         ...
         var __datossplit = datos.split("&");
         for (var i = 0; i < __datossplit.length; i++)
            if(__datossplit[i].split("=")[0] == "idpos")
               _ipos = __datossplit[i].split("=")[1];
      }
            ...
            var   a = new XMLHttpRequest(); 
                  ...
                  a.onreadystatechange = function()
                  {
                     if(a.readyState == 4)
                     {
                        if(a.status == 200)
                        {
                           if(((url.indexOf(".html") >-1) || (url.indexOf(".php") >-1)) && ((url.indexOf("funciones.php") == -1) && (url.indexOf("__chat.php") == -1) && (url.indexOf("__subearchivo.php") == -1)))
                           {
                              j = a.responseText
                              _recibe_ajax_maquetado(j, _esp)
                           }
                           else
                           {
                              try
                              {
                                 var aa = JSON.parse(a.responseText);
                                     aa._ipos = _ipos;
                                 (url.indexOf("__chat.php") > -1 || url.indexOf("__subearchivo.php") > -1) ? _recibe_ajax_chat(aa,_esp) : _recibe_ajax(aa,_esp);
                              }
                              catch(error)
                              {
                                 console.warn(error);
...
/* _________________________________________________________________________________________________________
   ...
   $r.validacion = {
      ...
      email:   function(n) {var _res = false; if(n.indexOf("@") > -1) if(n.indexOf("@.") == -1) if(n.split("@")[1].length > 3 ) _res = true; return  _res; },
   ...
   window.alert   = function(t,tiempo, elem)
   {
      ...
      var _contenedor_modal       = document.createElement("div");
      var _titulo_modal           = document.createElement("span");
      ...
      _titulo_modal.addEventListener('click',function(){
      ...
      clearTimeout($r._mata_modal);
      var _mata_modal = function() { if(document.getElementById("_modal_rd") != null) document.getElementById("_modal_rd").remove(); }
      $r._mata_modal = setTimeout(_mata_modal, tiempo);
