...
class MP {
    ...
    function __construct() {
        ...
        if ($i > 2 || $i < 1) {
            return  MercadoPagoException("Invalid arguments. Use CLIENT_ID and CLIENT SECRET, or ACCESS_TOKEN");
        }
    ...
    public function get_access_token() {
        ...
        if ($access_data["status"] != 200) {
            return  MercadoPagoException ($access_data['response']['message'], $access_data['status']);
        }
...
class MPRestClient {
    ...
    private static function build_request($request) {
        if (!extension_loaded ("curl")) {
            return  MercadoPagoException("cURL extension not found. You need to enable cURL in your php.ini or another configuration you have.");
        }

        if (!isset($request["method"])) {
            return  MercadoPagoException("No HTTP METHOD specified");
        }

        if (!isset($request["uri"])) {
            return  MercadoPagoException("No URI specified");
        }
            ...
            if ($json_content) {
                ...
                if(function_exists('json_last_error')) {
                    ...
                    if ($json_error != JSON_ERROR_NONE) {
                        return  MercadoPagoException("JSON Error [{$json_error}] - Data: ".$request["data"]);
                    }
                }
    ...
    // private static function exec($method, $uri, $data, $content_type) {
        ...
        if ($api_result === FALSE) {
            return  MercadoPagoException (curl_error ($connect));
        }
        ...
        if ($response['status'] >= 400) {
            ...
            //return  MercadoPagoException ($message, $response['status']);
        }
